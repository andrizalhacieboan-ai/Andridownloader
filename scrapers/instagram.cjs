...
// [UPDATE] Tambahkan dukungan stories & reels di regex
function extractShortcode(input) {
  input = input.trim();
  if (!input.includes('/')) return input;
  const m = input.match(/instagram\.com\/(?:p|reel|tv|stories|reels)\/([A-Za-z0-9_-]+)\/?([A-Za-z0-9_-]+)?/);
  if (!m) return null;
  
  // Untuk URL /stories/{username}/{id}, ambil ID (m[2])
  if (input.includes('/stories/') && m[2]) return m[2];
  
  return m[1];
}

// [UPDATE] Tambahkan parameter fetchUrl agar bisa menyesuaikan format Reels/Stories
async function fetchViaHtml(client, shortcode, fetchUrl) {
    ...
    try {
      ...
      for (const req of bbox.require) {
        ...
        const media = inner?.result?.data?.xig_polaris_media
          ...
          || inner?.result?.data?.xig_polaris_story; // Tambahkan polaris_story untuk Stories
        if (!media) continue;

        const item = media.if_not_gated_logged_out || media.item || media;
...
// [UPDATE] Tentukan URL fetch asli berdasarkan tipe (Reels/Stories/Post)
async function scrapeInstagram(inputUrl) {
  ...
  const isStory = inputUrl.includes('/stories/');
  const isReel = inputUrl.includes('/reel/') || inputUrl.includes('/reels/');
  
  // Gunakan URL asli jika stories/reels agar tidak kena redirect block dari IG
  const fetchUrl = isStory 
    ? inputUrl 
    : isReel 
      ? `https://www.instagram.com/reel/${shortcode}/`
      : `https://www.instagram.com/p/${shortcode}/`;
      
  const { item } = await fetchViaHtml(client, shortcode, fetchUrl);
...
module.exports = { scrapeInstagram, downloadMedia };
